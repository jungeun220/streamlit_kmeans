...
def main():
    ...
    if file is not None :  
        ...
        # 1-2. 10개 미만의 파일을 올리면, 에러 처리
        print(df.shape)
        if df.shape[0] < 10 :
            st.error('데이터의 갯수는 10개 이상이어야 합니다.')   
            return
        
        radio = st.radio('선택하세요', ['데이터프레임', '클러스터링'])
        if radio == '데이터프레임' :
